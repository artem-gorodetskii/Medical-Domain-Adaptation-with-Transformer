...
def run_synthesis(in_dir, out_dir, model_dir, hparams):
    ...
    with open(metadata_filename, encoding="utf-8") as f:
        ...
        print("Loaded training metadata for {} examples ({:.2f} hours)".format(len(metadata), hours))
    ...
    print("Synthesized training mel spectrograms at {}".format(synth_dir))
    return meta_out_fpath

def run_val_synthesis(in_dir, out_dir, model_dir, hparams):
    synth_dir = os.path.join(out_dir, "mels_gta")
    os.makedirs(synth_dir, exist_ok=True)
    metadata_filename = os.path.join(in_dir, "validation.txt")
    print(hparams_debug_string())
    ...
    with open(metadata_filename, encoding="utf-8") as f:
        ...
        frame_shift_ms = hparams.hop_size / hparams.sample_rate
        hours = sum([int(x[4]) for x in metadata]) * frame_shift_ms / 3600
        print("Loaded validation metadata for {} examples ({:.2f} hours)".format(len(metadata), hours))
    ...
    print("Synthesized validation mel spectrograms at {}".format(synth_dir))
    return meta_out_fpath
