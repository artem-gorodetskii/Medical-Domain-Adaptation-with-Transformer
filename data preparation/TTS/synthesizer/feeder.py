...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, val_metadata_filename, hparams):
		...
		with open(metadata_filename, encoding="utf-8") as f:
			...
			log("Loaded train metadata for {} examples ({:.2f} hours)".format(len(self._metadata), hours))

		# Load validation metadata
		self._val_mel_dir = os.path.join(os.path.dirname(val_metadata_filename), "mels")
		self._val_embed_dir = os.path.join(os.path.dirname(val_metadata_filename), "embeds")
		with open(val_metadata_filename, encoding="utf-8") as f:
			self._val_metadata = [line.strip().split("|") for line in f]
			frame_shift_ms = hparams.hop_size / hparams.sample_rate
			hours = sum([int(x[4]) for x in self._val_metadata]) * frame_shift_ms / (3600)
			log("Loaded validation metadata for {} examples ({:.2f} hours)".format(len(self._val_metadata), hours))
		...
		train_indices = np.arange(len(self._metadata))
		test_indices = np.arange(len(self._val_metadata))
		...
		self._test_meta = list(np.array(self._val_metadata)[test_indices])
	...
	def _get_test_groups(self):
		...
		mel_target = np.load(os.path.join(self._val_mel_dir, meta[1]))
		#Create parallel sequences containing zeros to represent a non finished sequence
		token_target = np.asarray([0.] * (len(mel_target) - 1))
		embed_target = np.load(os.path.join(self._val_embed_dir, meta[2]))
