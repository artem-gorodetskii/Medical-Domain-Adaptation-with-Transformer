...
def add_eval_stats(summary_writer, step, linear_loss, before_loss, after_loss, stop_token_loss,
                   loss, eval_mean_similarity, train_mean_similarity):
    values = [
    ...
    values.append(tf.compat.v1.Summary.Value(tag="Tacotron_eval_model/eval_stats/eval_similarity",
                                                 simple_value=eval_mean_similarity)) ### added

    values.append(tf.compat.v1.Summary.Value(tag="Tacotron_eval_model/eval_stats/train_similarity",
                                                 simple_value=train_mean_similarity)) ### added
...
def train(log_dir, args, hparams):
    ...
    val_metadat_fpath = os.path.join(args.validation_synthesizer_root, "validation.txt")
    
    log("Checkpoint path: {}".format(checkpoint_fpath))
    log("Loading training data from: {}".format(metadat_fpath))
    log("Loading validation data from: {}".format(val_metadat_fpath))
    ...
    with tf.compat.v1.variable_scope("datafeeder") as scope:
        feeder = Feeder(coord, metadat_fpath, val_metadat_fpath, hparams)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        try:
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                if step % args.eval_interval == 0 or step == 100 or step == 500 or step == 1500:
                    ...
                    if hparams.predict_linear:
                        ...
                        # for validation data
                        for i in tqdm(range(feeder.test_steps)):
                            eloss, before_loss, after_loss, stop_token_loss, mel_p_batch, mel_t_batch, t_len_batch,\
							align_batch, spk_emb_batch = sess.run( 
                                [
                                    eval_model.tower_loss[0], eval_model.tower_before_loss[0],
                                    eval_model.tower_after_loss[0],
                                    eval_model.tower_stop_token_loss[0],
                                    ...
                                    eval_model.tower_speakers_embedings
                                ])
                            ...
                            if i<6: # just for three batches

                                eval_batch_similarities = []

                                for j in range(hparams.tacotron_batch_size):

                                    mel_p = mel_p_batch[j]
                                    mel_t = mel_t_batch[j] 
                                    t_len = t_len_batch[j] 
                                    align = align_batch[j]
                                    spk_emb = spk_emb_batch[j]  

                                    generated_wav = audio.inv_mel_spectrogram(mel_p.T, hparams) 
                                    ...
                                    eval_batch_similarities.append(np.inner(spk_emb, generated_embeding))

                                eval_similarities.append(sum(eval_batch_similarities) / hparams.tacotron_batch_size)
                    ...
                    eval_mean_similarity = sum(eval_similarities) / len(eval_similarities)


                    # calculate similarity for training data on one batch
                    log("\nRunning calculation of similarity for six train batches at step {}".format(step))
                    for i in tqdm(range(6)): ### added
                        train_mel_p_batch, train_spk_emb_batch = sess.run( 
                                                                          [
                                                                           model.tower_mel_outputs[0],
                                                                           model.tower_speakers_embedings 
                                                                          ])
                        train_batch_similarities = []

                        for j in range(hparams.tacotron_batch_size):

                            train_mel_p = train_mel_p_batch[j]
                            train_spk_emb = train_spk_emb_batch[j]

                            generated_wav = audio.inv_mel_spectrogram(train_mel_p.T, hparams) ### added
                            generated_wav = encoder.preprocess_wav(generated_wav)
                            embed, partial_embeds, _ = encoder.embed_utterance(generated_wav, return_partials=True)
                            generated_embeding = np.mean(partial_embeds, axis=0)
                            generated_embeding = generated_embeding / np.linalg.norm(generated_embeding, 2) 
                            train_batch_similarities.append(np.inner(train_spk_emb, generated_embeding))

                        train_similarities.append(sum(train_batch_similarities) / hparams.tacotron_batch_size)

                        #if i>2: break # only for three batches

                    train_mean_similarity = sum(train_similarities) / len(train_similarities)
